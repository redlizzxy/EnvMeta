...
| 新手上手学习曲线 | N/A（读者要自己查 11 种上游工具文档 + 论文方法 + R 示例）| 10+（文献 + 配套脚本 + 样本 ID 对齐踩坑）| **~1-3 天**（首次） | ✅ S8-ux 新手落地包 | 3（加载样例 + 选问题 + 看解读 expander）| **~10 min** | 2026-04-17 |

> **一键复现**列：记录"他人拿到代码/项目后能否一键复现该图"（是/否/需配置环境）。
> **新手上手学习曲线**行：衡量**非生信研究生**从"拿到测序公司数据"到"跑出第一张图"的总耗时。
> S8-ux 新手落地包（data_preparation_zh.md + 图表选择向导 + 14 张图解读 expander + 样例数据一键加载）
> 把这个过程从"自己读文档 + 踩坑"缩短到"加载示例 → 选问题 → 看解读"10 分钟闭环。
