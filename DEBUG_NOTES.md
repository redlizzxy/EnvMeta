# DEBUG_NOTES — 开发日志与历史快照

> **本文件的角色**：EnvMeta 开发过程的 session 日志、阶段性自检、技术决策记录。
>
> 从 CLAUDE.md 拆分（2026-04-19）以保持核心规范文件简洁。
>
> - **项目核心规范** → 看 [CLAUDE.md](CLAUDE.md)
> - **历史日志 / 自检报告 / 踩坑记录** → 本文件
> - **未实现功能 / 投稿路线** → CLAUDE.md 的 "当前进度 + Backlog" 小节
>
> 新 session 开始时 Claude Code 自动读取 CLAUDE.md，不读本文件。需要回溯历史时
> 明确告诉 Claude "读 DEBUG_NOTES.md 里 YYYY-MM-DD 那段"。
...
#### T2 v1 用户实测反馈（晚 session 归档）

用户浏览 v1 HTML 后提 5 条反馈（均已整理到 plan 文件「T2-ε HTML v1 实测
反馈精调」章节，下次 session 落地）：

| # | 问题 | 性质 | 修复工时 |
|---|---|---|---|
| Q1 | 节点选择标准（completeness / top-N / ranking）用户不可见 | UX | ~30 min |
| Q2 | 环境相关 tab 空白（只有 T2-β 实现占位符）| 🔴 功能遗漏 | ~1-1.5h |
| Q3 | 跨元素耦合锚点 = 象限中心（粗粒），丢失化学物层信息 | UX / 精度 | ~1.5-2h |
| Q4 | 假说评分顶部是单组 overall，有分组时应先展示跨组 | 信息架构 | ~30-45 min |
| Q5 | 跨组对比 tab 含义不清（与假说评分跨组混淆）| 命名 / 说明 | ~45 min |

**Q1-Q5 修复思路**（plan 文件详述）：
- Q1: meta 块加"筛选标准"描述行 + 参数 tab 友好注解
- Q2: 新 `renderEnvPanel()` 渲染 env 表 + confidence 徽章 + full_corr_matrix
  折叠 + sensitivity 行 + claim 点击穿透 env_factor
- Q3: `chemicalToPathway(species)` 函数按 gene.substrate/product 匹配找锚点
  pathway 集群；耦合中点加产物节点（如 `As₂S₃` 小方块 + tooltip）
- Q4: 条件分支：`hypothesis_by_group_summary` 存在时跨组卡片组（CK/A/B 并排
  overall/label/null_p）置顶；单组评分改折叠 expander
- Q5: Tab 改名「🆚 通路 × 组对比」+ 顶部说明横幅 + `top_mag_genus` 跨组变化
  行黄底 + 按差值排序按钮 + "只看承载者切换" checkbox + element 过滤

**关键洞察**：Q2 暴露出 v1 的"功能完整度"问题 — 5 tab 里有 1 个空白（20%），
给人"未完工"感。下次 session **优先保 Q2**（功能遗漏 >> UX 优化）。

#### 下一步（Sunday Sprint 第 3 块 + T2-ε）

- **T2-ε HTML v1 精调（~4-5h）**：周六下午做。5 条反馈全部处理
- **T3 评测表电子版（~2-3h）**：周日上午做。10 任务腾讯问卷 + SUS 10 题中文版。
  发师弟师妹测试

合计剩余 ~6-8h，周日 18:00 前应能完成。
