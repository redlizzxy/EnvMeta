...
> 完整 schema + claim 类型 + 字段约束以代码 `envmeta/geocycle/hypothesis.py`
> 中的 dataclass 为准；本指南附录 §A 列了一份精简速查表。
...
> 该方法论自我批评见 Paper 3 methods 论文（投稿中；archived locally under
> `software/papers/paper3_envmeta/paper_archive/manuscript/scoring_validation_self_critique.md`）
> — 只写 backbone 通路 claim 是 calibration 不是 discrimination。
...
> 完整 schema 以源码 `envmeta/geocycle/hypothesis.py` 中的 dataclass + 各
> claim 类型 evaluator 为准。
...
"因果证实"。背后的方法论分析在 Paper 3 methods 论文（投稿中；本地 archive：
`software/papers/paper3_envmeta/paper_archive/manuscript/hypothesis_scoring_analysis.md`）。
...
> Paper 3 提交时所有示例 YAML 会随 Fork Bundle 发布（Zenodo DOI），届时
> 公开可下载。当前作者本机 archive 路径：

| 文件 | 角色 |
|---|---|
| `software/papers/paper3_envmeta/paper_archive/hypotheses/arsenic_steel_slag.yaml` | 单层 calibration 假说示例（v1，仅 calibration）|
| `software/papers/paper3_envmeta/paper_archive/benchmarks/external/liu_2023_coldseep/liu2023_hypothesis.yaml` | 单层 calibration（pre-registered）|
| `software/papers/paper3_envmeta/paper_archive/benchmarks/external/*/<*>_hypothesis_stress.yaml` | 双层 calibration+stress（v0.9 引入）|

最简模板（直接 copy 到新 YAML）：

```yaml
name: my_hypothesis_v1
description: |
  环境背景一句话 + 假说核心机制一句话
claims:
  - id: backbone_pathway_active
    type: pathway_active
    weight: 1.0
    required: true                   # backbone → veto on zero
    params:
      pathway: "Arsenate reduction"   # 改成你的目标 pathway
      min_n_active: 3
      min_completeness: 30
    description: |
      [REASONING] 该环境地球化学层面预期此通路存在
      [PREDICTION] expected_label=strong
      [LITERATURE] 引用 ≥ 5 年前综述 DOI
  - id: domain_mismatch_stress
    type: pathway_active
    weight: 0.5
    params:
      pathway: "Methanogenesis (CO2 reduction)"  # 改成该环境不该有的 pathway
      min_n_active: 3
      min_completeness: 30
    description: |
      [REASONING] cross-topic stress — 该环境不应有产甲烷主导
      [PREDICTION] expected_label=insufficient
```
